...
var Faceplate = function(options) {
  ...
  this.parse_signed_request = function(signed_request, cb) {
    ...
    var req = request('https://graph.facebook.com/oauth/access_token',
...
var FaceplateSession = function(plate, signed_request) {
  ...
  this.fql = function(query, cb) {
    ...
    var req = request('https://api.facebook.com/method/'+method,
      { qs: params }, function(err, resp, body){
        if(err) return cb(err);
        onComplete(body);
      });
  ...
  this.post = function (params, cb) {
    var req = request(
      ...
      {qs: {access_token: self.token}, form: params, method:'POST'},
